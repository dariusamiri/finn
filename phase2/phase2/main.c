...
void product_token()
{
    srand((int)time(0));
    for (int i = 0; i < 32; i++) {
        AuthToken[i] = rand()%(num_online_user+1) + 79;
    }
}
...
void Channel_Members(char* buffer)
{
    ...
    cJSON* recog_is_join_in_channel_by_auth = cJSON_GetObjectItem(token_get_channel, get_online_token_arr_char);
        ...
        if (strcmp(recog_channel_by_auth_str, recog_is_join_in_channel_by_auth_str)) {
